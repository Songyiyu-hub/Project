...
model = Model("MultiDriverScheduling")
Y = model.addVars(m, n, n, vtype=GRB.BINARY, name="Y")
C = model.addVars(m, n, vtype=GRB.CONTINUOUS, name="C")
Cmax = model.addVar(vtype=GRB.CONTINUOUS, name="Cmax")

for i in range(n):
    model.addConstr(quicksum(Y[k, i, r] for k in range(m) for r in range(n)) == 1)

for k in range(m):
    for r in range(n):
        model.addConstr(quicksum(Y[k, i, r] for i in range(n)) <= 1)

for k in range(m):
    for r in range(1, n):
        model.addConstr(
            quicksum(Y[k, i, r] for i in range(n)) <=
            quicksum(Y[k, i, r - 1] for i in range(n))
        )

for k in range(m):
    for i in range(n):
        rhs = ((b + 1)**2) * (t0 + theta[i]) + (b + 2) * T[i] + (b + 1) * Z[i]
        model.addConstr(
            C[k, i] + (1 - Y[k, i, 0]) * U >= rhs
        )
...
for k in range(m):
    for i in range(n):
        for l in range(n):
            for r in range(1, n):
                rhs = ((b + 1)**2) * (C[k, l] + theta[i]) + (b + 2) * T[i] + (b + 1) * Z[i]
                model.addConstr(
                    C[k, i] + (2 - Y[k, i, r] - Y[k, l, r - 1]) * U >= rhs
                )


for k in range(m):
    for i in range(n):
        model.addConstr(Cmax >= C[k, i])
...
if model.status == GRB.OPTIMAL:
    print(f"Minimized maximum completion time: {Cmax.X:.2f}")
    for k in range(m):
        assigned_tasks = [i+1 for i in range(n) for r in range(n) if Y[k, i, r].X > 0.5]
