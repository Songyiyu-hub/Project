...
theta_sorted = list(reversed(theta))
T_sorted = list(reversed(T))
Z_sorted = list(reversed(Z))
print("Tasks sorted in non-increasing order and their corresponding sorting values S_i:")
print("{:<8} {:>8} {:>8} {:>8} {:>12}".format("Task ID", "θ_i", "T_i", "Z_i", "S_i"))

for i in range(n):
    S_i = (b + 1)**2 * theta_sorted[i] + (b + 2) * T_sorted[i] + (b + 1) * Z_sorted[i]
    print("task_{:<3} {:>8.3f} {:>8.3f} {:>8.3f} {:>12.6f}".format(
        i + 1, theta_sorted[i], T_sorted[i], Z_sorted[i], S_i))

def compute_lb(n, m, t0,sorted_theta, sorted_T, sorted_Z, b):
    ceiling = math.ceil(n / m)
    floor = math.floor(n / m)
    print(f"Ceiling: {ceiling},Floor: {floor}")
    term_1 = (b + 1) ** (2 * ceiling) * t0 * (n - floor * m) + \
             (b + 1) ** (2 * floor) * t0 * (m - (n - floor * m))
    print(f"First term calculation result: {term_1}")
    term_2 = 0
    for i in range(n):
       term_2 += ((b + 1) ** 2 * sorted_theta[i] + (b + 2) * sorted_T[i] + (b + 1) * sorted_Z[i]) * \
                      (b + 1) ** (2 * (math.ceil((i + 1) / m) - 1))
    print(f"Second term calculation result: {term_2}")
    LB = (1 / m) * term_1 + (1 / m) * term_2
    return LB

LB = compute_lb(n, m, t0, theta_sorted, T_sorted, Z_sorted, b)
print("\nComputed lower bound LB: {:.2f}".format(LB))
